...
# Create a fake dataset for 100k users with name, state, birthdate and device_id
...
# change the birthdate to datetime
faker_data['user_birth_date'] = pd.to_datetime(
    faker_data['user_birth_date'])
print(faker_data.dtypes)

# read the businesses dataset (10k rows)
businesses = pd.read_json(my_dir / "sf_businesses.json", lines=True)
print(businesses)
print(businesses.dtypes)

# create (10.000 * 100) businesses --> 1M rows by concatenating the same dataset multiple times
df_repeated = pd.concat([businesses] * 100, ignore_index=True)
print(df_repeated.shape)

# randint needs to be the same as the number of users --> 100k
...
df_repeated2.to_parquet(my_dir / 'sf_appscans.parquet.gzip', compression='gzip')
