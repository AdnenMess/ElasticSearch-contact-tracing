...
if text:
    ...
    # Search the index.
    # 1k is enough to find all the businesses.
    ...
    df = df.filter(
        items=['_source.business_id', '_source.business_name', '_source.business_address', 'latitude', 'longitude',
               '_source.zip'], axis=1)
    ...
    table_df = df.filter(
        items=['_source.business_id', '_source.business_name', '_source.business_address', '_source.zip'], axis=1)

    # fix names before printing the table
    table_df = table_df.rename(columns={"_source.business_id": "Business ID", "_source.business_name": "Name",
                                        "_source.business_address": "Address", "_source.zip": "Postal Code"})
    ...
    folium_map = folium.Map(location=[df.iloc[0]['latitude'], df.iloc[0]['longitude']], zoom_start=13)

    # add the markers
    for index, row in df.iterrows():
        folium.Marker(
            [row['latitude'], row['longitude']],
            popup=f"{row['_source.business_name']} <br> ID= {row['_source.business_id']}").add_to(folium_map)
...
if postal_code:
    ...
    # Search the index.
    # 1k is enough to find all the businesses.
    # The problem is that it wants to return all documents
    # for this query for you There is no group by query for strings (we just uploaded it as string)
    res = es.search(index="my_app_scans", body=query_body, size=1000)
    ...
    for index, row in df.iterrows():
        folium.Marker(
            [row['latitude'], row['longitude']],
            popup=f"{row['_source.business_name']} <br> ID= {row['_source.business_id']}").add_to(m)
...
if business_id:
    ...
    table_df = df.filter(
        items=['_source.scan_timestamp', '_source.deviceID', '_source.user_name', '_source.user_birth_date'], axis=1)

    # turn the epochs into timestamps
    table_df['_source.user_birth_date'] = table_df['_source.user_birth_date'].apply(
        lambda s: datetime.datetime.fromtimestamp(s / 1000000).strftime("%Y/%m/%d"))
    table_df['_source.scan_timestamp'] = table_df['_source.scan_timestamp'].apply(
        lambda s: datetime.datetime.fromtimestamp(s / 1000000).strftime("%Y/%m/%d %H:%M:%S"))
...
# Below the fist chart add an input field for the invoice number
...
if device_id:
    ...
    # filter only the needed columns for the table
    table_df = df.filter(
        items=['_source.scan_timestamp', '_source.business_id', '_source.business_name', '_source.business_address',
               'longitude', 'latitude'], axis=1)  # ,'latitude','longitude'
    ...
    table_df = table_df.rename(
        columns={"_source.scan_timestamp": "Scan Timestamp", "_source.business_id": "Business ID",
    ...
    for index, row in df.iterrows():
        folium.Marker(
            [row['latitude'], row['longitude']], popup=row['_source.scan_timestamp'],
            tooltip=row['_source.business_name']
        ).add_to(m)
